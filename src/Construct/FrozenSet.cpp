...
std::vector<double> construct::DensityEvolutionGA(size_t nLayers, double snr)
{
    static const auto even = [](double x) {
        ...
        if (x > 3.5) {
            return x * (9.0047e-3 * x + 0.76943) - 0.95068;
        }
        if (x > 1) {
...
std::vector<bool> construct::BuildFrozenSet_Bhattacharyya(double p, size_t nLayers, size_t dimension)
{
    return BuildFrozenSet(BhattacharyyaParameters(p, nLayers), dimension);
}

std::vector<bool> construct::BuildFrozenSet_GA(double snr, size_t nLayers, size_t dimension)
{
