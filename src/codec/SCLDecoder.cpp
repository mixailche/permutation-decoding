...
using codec::SCLDecoder;
using codec::PolarSpecification;

SCLDecoder::SCLDecoder(const PolarSpecification* spec, size_t maxPaths)
    : mSpec(spec)
    , mNumLayers(utils::IntLog2(spec->Length))
    , mMaxPaths(maxPaths)
    , mPathMetrics(maxPaths)
    , mPathLLRs(mNumLayers + 1, std::vector<double*>(maxPaths))
    , mPathSymbols(mNumLayers + 1, std::vector<bool*>(maxPaths))
    , mInactiveArrayIndices(mNumLayers + 1)
    , mInactivePathIndices()
    , mIsActivePath(maxPaths, false)
    , mArrayReferenceCount(mNumLayers + 1, std::vector<size_t>(maxPaths, 0))
    , mPathToArray(mNumLayers + 1, std::vector<size_t>(maxPaths))
...
void SCLDecoder::Initialize() const
{
    ...
    for (size_t layer = 0; layer <= mNumLayers; mInactiveArrayIndices[layer++].pop_back()) {
...
double SCLDecoder::GetContinuedPathMetric(double currentMetric, double y, bool u) const
{
    bool hasPenalty = u != y < 0;
    mNumOperations += hasPenalty;
    return currentMetric + hasPenalty * std::abs(y);
}
...
void SCLDecoder::ContinuePaths_Unfrozen(size_t phase) const
{
    ...
    auto indices = utils::SortingPerm(forks, [&](double x, double y) {
        mNumOperations++;
        return x < y;
    });
...
std::vector<bool> SCLDecoder::Decode(const std::vector<double>& inputLLRs) const
{
    Initialize();
    ...
    for (size_t pathIdx = 0; pathIdx < mMaxPaths; pathIdx++) {
        auto pathMetric = mPathMetrics[pathIdx];
        if (mIsActivePath[pathIdx] && ++mNumOperations && pathMetric < bestMetric) {
            bestPathIdx = pathIdx;
            bestMetric = pathMetric;
        }
    }

    auto bestPathSymbols = GetSymbolsArray(mNumLayers, bestPathIdx);
